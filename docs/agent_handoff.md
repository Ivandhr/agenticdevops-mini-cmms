...
**T-003 specced 2026-07-22 — persistence foundation.** `docs/tasks/task_T-003_persistence-foundation.md`: SQLAlchemy 2.0 models for the four tables, Alembic wired dual-engine, one reversible initial migration, the partial unique index. No endpoints, no auth, no domain services, no UNS. **Branches from `main` after T-002 merges** so its PR runs against a real CI workflow.

**New in this task — a Linux CI job, and a new decision:**
...
**T-002 specced and ready to run (2026-07-22).** `docs/tasks/task_T-002_renderer-scaffold-ci.md` — Electron/Vite/React/TS renderer skeleton plus `.github/workflows/ci.yml`. It closes the Rule 12 loop T-001 left open (the TypeScript leg of `GET /health`), and it is the task that makes "CI green" true for the first time. Spec-decided constraints the coding agent must not re-decide: CI on **`windows-latest`** with Node 22 / Python 3.12 and `contents: read`; `ruff` and `mypy` stay **soft** per the runbook's promotion criterion; backend base URL is the literal `http://127.0.0.1:8000` — **not `localhost`**, which on Windows can resolve to `::1` while uvicorn binds IPv4 and presents as a dead backend; `package-lock.json` committed, since CI runs `npm ci`; no Electron launch or packaging step in CI. The task implements **no FR** — it is infrastructure, and the spec says so rather than citing a requirement it doesn't satisfy.
...
1. **Human: run T-002.** Spec is written (`docs/tasks/task_T-002_renderer-scaffold-ci.md`) and the index row is in. Cut branch `T-002-renderer-scaffold-ci`, hand the coding-agent command to Claude Code, then Cursor QA → PM read-verify → human runtime test → PM close-out → PR. **Branch→PR discipline resumes here** — this is the task that makes CI real, so it is the last one that could be excused from a PR.
2. **Human: run T-003 after T-002 merges.** Spec written, index row in. Branch `T-003-persistence-foundation`.
3. **Human: answer the two `data-model.md` § 7 open items** — whether `priority` is a free string or an ordered set with sorting semantics in the Planner queue, and (with the auth task) the password-hashing choice. Neither blocks T-003; `priority` ships as a free `String` and can tighten later.
4. **Human: decide on `docs/design-guide.md`** — author it, or leave it *(to author)*. Blocks the first real UI task, not T-002 or T-003.
5. **PM: author `docs/uns-contract.md`** — gates FR-010–FR-014 and FR-020. Can wait; the manual ingress path (FR-021) is specced to work with no broker, so the first usable slice does not depend on it.
6. Human: sync the constitution edits into the Claude Project instructions field (if still relevant while the PM runs as Claude Code). Now **six**: api-contract Tier 2, Rule 12 repoint, §3 review-agent config path, `functional-spec.md` in Tier 1, `design-guide.md` marked *(to author)*, §2's repository path corrected to the Windows ground truth, and `data-model.md` flipped to live.
