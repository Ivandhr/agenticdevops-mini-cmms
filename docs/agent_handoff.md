...
2. **PM: spec the persistence foundation task (T-003)** — SQLAlchemy + Alembic wired dual-engine, the four tables from `data-model.md`, the naming convention and `render_as_batch=True`, and the partial unique index. Unblocked now that the data model is authored; can be specced while T-002 is in flight.
3. **Human: answer the two `data-model.md` § 7 open items** — whether `priority` is a free string or an ordered set with sorting semantics in the Planner queue, and (with the auth task) the password-hashing choice.
