...
2. **PM: author `docs/data-model.md`** — gates every persistence task, which is the entire next phase. Should start during/after T-002 rather than waiting for it.
3. **Human: decide on `docs/design-guide.md`** — author it, or leave it *(to author)*. Blocks the first real UI task, not T-002.
...
5. Human: sync the constitution edits into the Claude Project instructions field (if still relevant while the PM runs as Claude Code). Now **five**: api-contract Tier 2, Rule 12 repoint, §3 review-agent config path, `functional-spec.md` in Tier 1 with `design-guide.md` marked *(to author)*, and §2's repository path corrected to the Windows ground truth.
